...
import { ViewState, Match, Post, Sport } from './types';
import HomeView from './views/HomeView';
import MatchesView from './views/MatchesView';
import CommunityView from './views/CommunityView';
import { getAiMatchRecommendations, getCommunityFeed } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const NavLink = ({ id, label }: { id: ViewState, label: string }) => (
    <button
      onClick={() => setView(id)}
        ...
        {view === 'MYPAGE' && (
          <div className="py-20 text-center animate-in fade-in duration-500 bg-white rounded-3xl border border-slate-200 shadow-sm max-w-md mx-auto">
             ...
             <div className="mt-8 px-8 space-y-3">
               <button className="w-full py-3 bg-[#FEE500] text-[#191919] rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
                 <span className="text-lg">💬</span> 카카오로 3초만에 시작하기
               </button>
               <button className="w-full py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition-colors">
                 다른 방법으로 로그인
               </button>
             </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-10">
          <div className="col-span-1 md:col-span-2">
             ...
             <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
               아마추어 스포츠인들을 위한 데이터 기반 매칭 및 분석 플랫폼. 당신의 스포츠 라이프를 한 단계 업그레이드하세요.
             </p>
          </div>
          <div>
            ...
            <ul className="text-slate-500 text-sm space-y-2">
              ...
              <li className="hover:text-indigo-600 cursor-pointer">구장 예약</li>
            </ul>
          </div>
          <div>
            ...
            <ul className="text-slate-500 text-sm space-y-2">
              ...
              <li className="hover:text-indigo-600 cursor-pointer">제휴 문의</li>
            </ul>
          </div>
        </div>
